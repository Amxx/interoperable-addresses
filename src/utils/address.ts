...
export function toChecksumAddress(address: BytesLike) {
  ...
  validateArgument(/^(0x)?[0-9a-fA-F]{40}$/i.test(address), `Given address "${address}" is not a valid Ethereum address`);
  ...
  for (let i = 0; i < 40; i += 2) {
    chars[i] = (hash[i >> 1]! >> 4) < 8
      ? chars[i]!.toLowerCase()
      : chars[i]!.toUpperCase();
    chars[i + 1] = (hash[i >> 1]! & 0x0f) < 8
      ? chars[i + 1]!.toLowerCase()
