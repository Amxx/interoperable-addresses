...
export function computeChecksum(self: InteroperableAddress) : string {
    return toHex(keccak256(toBytes(addressCoder.encode(self)).slice(2))).slice(2,10).toUpperCase();
}
...
export const addressCoder /* : Coder<InteroperableAddress, BytesLike> */ = {
    decode: (self: BytesLike) => {
        ...
        if (referenceLength > 0) output.reference = CAIP350[chainType].reference.encode(buffer.slice(5, 5 + referenceLength));
        if (addressLength > 0) output.address = CAIP350[chainType].address.encode(buffer.slice(6 + referenceLength, 6 + referenceLength + addressLength));
        output.checksum = computeChecksum(output);
        return output;
    },
    encode: ({ chainType, reference, address }: InteroperableAddress) => {
        var chainTypeBytes: Bytes, referenceBytes: Bytes, addressBytes: Bytes;
        switch (chainType) {
            case 'eip155':
            case '0x0000':
            {
...
export const nameCoder /* : Coder<InteroperableAddress, string> */ = {
    decode: (self: string, pedantic: boolean = false) => {
        const parsed = TypedRegEx('^((?<address>[.-:_%a-zA-Z0-9]*)@)?(?<chain>[.-:_a-zA-Z0-9]*)#(?<checksum>[0-9A-F]{8})$').captures(self);
        ...
        if (reference !== undefined) output.reference = CAIP350[chainType].reference.encode(reference);
        if (address !== '') output.address = CAIP350[chainType].address.encode(address);
